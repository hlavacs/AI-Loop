...
On Windows, the repository includes `start-ai-loop-with-email.cmd`. Keep personal mail settings outside the repository in a JSON file beside the repository folder (two levels above `ai-loop/`), named `start-ai-loop-with-email.json`. Copy the checked-in example and adapt it:
...
- `../../ai-runs/JOB_ID` (beside the repository): default isolated job worktree.
