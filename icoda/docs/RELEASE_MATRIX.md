...
The current source inventory contains 37 top-level Python modules under `icoda_core/`, counting every `*.py` file
including `__init__.py`, and 58 Python test modules under `tests/`, counting 54 top-level `test_*.py` modules plus
the 4 `*_acceptance.py` programs. Supporting test utilities and the sample project are excluded.
