...
The current source inventory contains 39 top-level Python modules under `icoda_core/`, counting every `*.py` file
including `__init__.py`, and 61 Python test modules under `tests/`, counting 56 top-level `test_*.py` modules plus the 5 `*_acceptance.py` programs. Supporting test utilities and the sample project are excluded.
