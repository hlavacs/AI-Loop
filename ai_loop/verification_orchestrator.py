...
def shape_dashboard_evidence(
    ...
    *,
    ...
    if isinstance(preview_limit, bool) or not isinstance(preview_limit, int) or preview_limit <= 0:
    ...
    if artifact_path is not None and (
        ...
        raise VerificationExecutionError("dashboard evidence has an invalid artifact path")
    textual = _textual_media_type(evidence.get("media_type"))
    preview = ""
    preview_truncated = False
    if textual:
        raw_preview = evidence.get("preview")
        if not isinstance(raw_preview, str) and artifact_path is None and "inline_value" in evidence:
            try:
...
def _shape_dashboard_repetition(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(evidence, list):
        raise VerificationExecutionError("dashboard repetition evidence must be an array")
    return {
...
def _dashboard_contracts(
    ...
    realization = str(summary.get("realization_state") or summary.get("status") or "unrealized")
    ...
    for index, target in enumerate(case.get("coverage_targets", []), 1):
        ...
        if not isinstance(target, Mapping):
            raise VerificationExecutionError("dashboard coverage target must be text or an object")
        ...
        evidence_entries.append(
            {
                ...
                "kind": kind,
                "enforcement": enforcement,
                "runtime": None if emitted is None else shape_dashboard_evidence(emitted),
            }
        )
    return {
...
def build_verification_dashboard_projection(
    ...
    *,
    ...
    cases = payload.get("verification")
    if not isinstance(cases, list):
        raise VerificationExecutionError("dashboard manifest must contain verification cases")
    summary_by_id: dict[str, Mapping[str, Any]] = {}
    for item in runtime_summary:
        verification_id = item.get("verification_id") if isinstance(item, Mapping) else None
        if not isinstance(verification_id, str) or not verification_id:
            raise VerificationExecutionError("dashboard summary requires verification IDs")
    ...
    for item in repetitions:
        ...
        if not isinstance(verification_id, str):
            raise VerificationExecutionError("dashboard repetition requires a verification ID")
    ...
    for item in manual_acknowledgements:
        verification_id = item.get("verification_id") if isinstance(item, Mapping) else None
        if not isinstance(verification_id, str):
            raise VerificationExecutionError("dashboard acknowledgement requires a verification ID")
    ...
    for raw_case in cases:
        if not isinstance(raw_case, Mapping) or not isinstance(
            ...
            raise VerificationExecutionError("dashboard manifest cases require verification IDs")
        ...
        if automation != summary.get("automation") or blocking != bool(summary.get("blocking")):
            raise VerificationExecutionError(
            ...
            attempts.append(
                {
                    ...
                    "status": (
                        ...
                        and all(item["status"] == RepetitionStatus.PASSED.value for item in attempt_repetitions)
                        else "failed"
                    ),
    ...
    if extras:
        raise VerificationExecutionError(
            "dashboard data contains cases absent from the manifest: " + ", ".join(sorted(extras))
        )
    return tuple(rows)
